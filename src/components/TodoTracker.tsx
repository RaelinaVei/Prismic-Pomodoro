...
export function TodoTracker({ userId, onSave }: Props) {
  ...
  const save = (items: Todo[]) => {
    ...
    if (onSave) {
      ...
      debounceRef.current = setTimeout(() => onSave(items), 1000);
    }
  };
            ...
            <div className="px-4 pb-4 space-y-2">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-transparent border border-white/20 rounded-lg px-3 py-2 text-sm font-body timer-text placeholder:text-white/40 focus:outline-none focus:border-white/50"
                />
                <button
                  onClick={addTodo}
                  className="p-2 rounded-lg bg-white/10 hover:bg-white/20 transition-colors timer-text"
                >
                    ...
                    >
                      <button
                        ...
                        className={`w-5 h-5 rounded-md border flex items-center justify-center flex-shrink-0 transition-colors ${
                          todo.done ? "bg-white/30 border-white/30" : "border-white/30 hover:border-white/60"
                        }`}
                      >
                        {todo.done && <Check className="w-3 h-3 timer-text" />}
                      </button>
                      <span className={`flex-1 text-sm font-body timer-text ${todo.done ? "line-through opacity-50" : ""}`}>
                        {todo.text}
                      </span>
                      <button
